		...
		if (String(page.name()) === 'Styles') {
			...
			page.layers().forEach((layer) => {
				if (layer.class() === MSTextLayer) {
					...
					if (String(layer.styleAttributes()['MSAttributedStringTextTransformAttribute']) === '1')
						textTransform = 'uppercase'; //  null: none, 1: uppercase and 2 lowercase
					if (String(layer.styleAttributes()['MSAttributedStringTextTransformAttribute']) === '2')
					...
					let weight = dom.fromNative(layer).style.fontWeight;
		...
		if (String(page.name()) === 'Colours' || String(page.name()) === 'Colors') {
			...
			page.layers().forEach((layer) => {
				DocumentColours[layer.name()] = convert
					? convertSketchColourToRGBA(layer.style().firstEnabledFill().color())
					: layer.style().firstEnabledFill().color();
			});
		}
	});
	...
	json.typography.forEach((item) => {
		Object.keys(json.colours).forEach((colour) => {
			item.alignments.map((align, index) => {
				...
				typeStyles.push({
					name: `${name[0]}/${colour}/${index + '_' + align + (name.length > 1 ? '/' + name[1] : '')}`,
					style: {
